...
public class HttpUtil {
    ...
    public static Map<String, Object> getRequestParameters() {
        Map<String,Object> params = WebUtils.getParametersStartingWith(HttpUtil.getRequest(),null);
        return params;
    }
    ...
    public static Integer getImage(String imageUrl){
        Integer image = -1;
        BufferedReader in = null;
        try {
            StringBuffer query = new StringBuffer();
            ...
            connection.connect();
            // 获取所有响应头字段
            image = connection.getHeaderFieldInt(HttpHeaders.CONTENT_LENGTH,-1);
        } catch (Exception e) {
            System.out.println("发送GET请求出现异常！" + e);
            e.printStackTrace();
        }
